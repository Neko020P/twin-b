...
class BuildingModel(Model):
    def __init__(self, config, agents_file=None, idf_path=None, ep_control=False):
        ...
        if self.ep_control:
            if idf_path:
                IDF.setiddname("/project/lt200291-ignite/Project_chomwong/energyplus/EnergyPlus-25.1.0-1c11a3d85f-Linux-CentOS7.9.2009-x86_64/Energy+.idd")
                idf = IDF(idf_path)
            ...
            def setup_handles_first_timestep(state):
                for zone in self.zones:
                    ...
                    if temp_handle == -1:
                        print(f"[WARN] ไม่พบ variable 'Zone Air Temperature' ของ zone '{zone}'")

                    sp_handle = self.exchange.get_actuator_handle(state, "Zone Temperature Control", "Cooling Setpoint", ep_zone)
                    if sp_handle == -1:
                        print(f"[WARN] ไม่พบ actuator 'Cooling Setpoints' ของ zone '{zone}'")
    ...
    def _create_agents_from_json(self):
        ...
        print(f"✅ โหลด agent สำเร็จทั้งหมด: {len(self.schedule.agents)} ตัว")
    ...
    def step_agents(self, ep_model=None):
        ...
        for agent in self.schedule.agents:
            ...
            self.agent_results.append({
                ...
                "comfort_level": round(agent.comfort_level, 2) if agent.comfort_level else None,
    ...
    def run_simulation_and_export(self, steps, zone_file="zone_results.csv", agent_file="agent_results.csv"):
        # if not self.ep_control:
        # # Mesa-only
        #     for _ in range(steps):
        #         self.step_agents(ep_model=None)
        for _ in range(steps):
